...
class Container(BaseContainer):
    ...
    def exposes(self) -> [ExposeLink]:
        port = self.manager.config.get("COLLABORA_SERVER_EXPOSE_PORT", type=int, default=0)
        return [
            self.expose_container("collabora-code-server", "MicrosoftOffice", "", self.load_port_url(port, https=False)),
        ]
